...
import {
	Activity, BatteryCharging, ChartColumn, ChartNetwork, Cpu,
	Fuel, Gauge, Power, SquareArrowRight, ThermometerSnowflake,
	TriangleAlert, WavesArrowDown, Wifi, Zap,
} from "lucide-react";
...
import { useMqttTelemetry, type MqttConnectionStatus } from "./../../hooks/useMqttTelemetry";
import {
	operationModeLabel,
	gensetStateLabel,
...
function buildPayload(
	...
	return {
		metrics: [
			{ id: 1,  label: "Funcionamento",    value: v(t?.hourmeter),         unit: "h",    icon: Activity,             color: "teal"   },
			{ id: 2,  label: "Partidas",          value: v(t?.starts),            unit: "",     icon: Power,                color: "pink"   },
			{ id: 3,  label: "Temp. Refrig.",     value: t?.coolantTemp ?? "–",   unit: "°C",   icon: ThermometerSnowflake, color: "yellow" },
			{ id: 4,  label: "Pressão Óleo",     value: v(t?.oilPressure),       unit: "KPA",  icon: WavesArrowDown,       color: "blue"   },
			{ id: 5,  label: "Modo de Operação",  value: operationModeLabel(t?.operationMode ?? null), unit: "", icon: SquareArrowRight, color: "orange" },
			{ id: 6,  label: "Rotação",           value: v(t?.rotation),          unit: "rpm",  icon: Gauge,                color: "teal"   },
			{ id: 7,  label: "Frequência",        value: t?.genFrequency ?? "–",  unit: "Hz",   icon: Wifi,                 color: "pink"   },
			{ id: 8,  label: "Tensão Bateria",    value: t?.battery ?? "–",       unit: "Vcc",  icon: BatteryCharging,      color: "yellow" },
			{ id: 9,  label: "Nível Combustível", value: "–",                     unit: "%",    icon: Fuel,                 color: "blue"   },
			{ id: 10, label: "Falha Ativa",       value: v(t?.activeFault),       unit: "",     icon: TriangleAlert,        color: "orange" },
		],
		...
		// Tensões (voltages)
		generatorCurrent: [
			{ label: "AN", value: v(t?.genPhaseAN), unit: "V", icon: Activity, color: "cyan"   },
			{ label: "BN", value: v(t?.genPhaseBN), unit: "V", icon: Activity, color: "cyan"   },
			{ label: "CN", value: v(t?.genPhaseCN), unit: "V", icon: Activity, color: "cyan"   },
			{ label: "AB", value: v(t?.genPhaseAB), unit: "V", icon: Activity, color: "cyan"   },
			{ label: "BC", value: v(t?.genPhaseBC), unit: "V", icon: Activity, color: "cyan"   },
			{ label: "CA", value: v(t?.genPhaseCA), unit: "V", icon: Activity, color: "cyan"   },
		],

		// Correntes (currents)
		generatorVoltage: [
			{ label: "Fase A", value: v(t?.genPhaseA), unit: "A", icon: Zap, color: "yellow" },
			{ label: "Fase B", value: v(t?.genPhaseB), unit: "A", icon: Zap, color: "yellow" },
			{ label: "Fase C", value: v(t?.genPhaseC), unit: "A", icon: Zap, color: "yellow" },
		],

		// Potências (power)
		generatorPower: [
			{ label: "Ativa",    value: v(t?.genActive),   unit: "kW",   icon: ChartColumn, color: "purple"  },
			{ label: "Reativa",  value: v(t?.genReactive), unit: "kVAR", icon: ChartColumn, color: "pink"    },
			{ label: "Aparente", value: v(t?.genApparent), unit: "kVA",  icon: ChartColumn, color: "fuchsia" },
			{ label: "Fat. Pot", value: v(t?.genFactor),   unit: "",     icon: ChartColumn, color: "orange"  },
		],
	};
}
...
export default function GeneratorView() {
	...
	useEffect(() => {
		...
		async function loadAll() {
			try {
				...
				const [gen, alarmsData, /*histData*/]: [GeneratorInfo, AlarmRow[], HistoryRow[]] =
					await Promise.all([genRes.json(), alarmsRes.json(), /*histRes.json()*/]);
	...
	const historyTableRows = useMemo(
		() =>
			history.map((h) => ({
				timestamp:     h.dateTime ? new Date(h.dateTime as string).toLocaleString("pt-BR") : "–",
	...
	return (
		<div className="relative max-w-7xl mx-auto px-4 py-6 space-y-5">
			...
			{dbLoading ? (
				...
				<DataTable2
					props={{
						columns: alarmTableRows.length > 0 ? alarmsColumns : historyColumns,
						data:    alarmTableRows.length > 0 ? alarmTableRows : historyTableRows,
					}}
				/>
			)}
